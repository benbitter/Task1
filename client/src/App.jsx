...
export default function App() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const res = await axios.get("http://localhost:3001/api/v1/auth/fetchUserData", {
...
const responseGoogle = async (authResult) => {
  try {
    ...
    const handleLogin = async () => {
      try {
        setLoading(true);
        const res = await axios.post("http://localhost:3001/api/v1/auth/login", login, {
          withCredentials: true,
        });
    ...
    const handleRegister = async () => {
      try {
        setLoading(true);
        const res = await axios.post("http://localhost:3001/api/v1/auth/register", reg);
